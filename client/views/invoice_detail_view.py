...
def build_view(page: ft.Page, invoice_id: int):
    ...
    # Sisa Hutang Toko = Modal (invoice_bon) - Lebih Uang + Kurang Uang
    # (formula CONFIRMED, sama seperti backend services/finance_service.py).
    # Sebelumnya: (invoice_bon or 0) + total_barang - total_uang -- ini
    # aljabar-nya SAMA (Lebih Uang = uang-barang saat positif), jadi angka
    # tidak berubah, cuma sekarang ditampilkan dengan warna & tanpa minus.
    sisa_hutang_toko = (invoice_bon or 0) + total_barang - total_uang
    # TODO (fase 2): "Sisa Barang di Toko" akan jadi input manual (dicek
    # fisik tiap hari), bukan disamakan dengan Sisa Hutang seperti sekarang.
    ...
    # Card Sisa Hutang Toko: merah kalau masih hutang, hijau kalau lunas,
    # nominal selalu ditampilkan positif (tidak minus).
