...
def build_view(page: ft.Page):
    ...
    # =========================================================================
    # DIALOG TAMBAH & EDIT PENGAMBILAN PABRIK
    # =========================================================================
    ...
    form_keterangan = ft.TextField(
        ...
        min_lines=2,
        max_lines=3,
        hint_text="Contoh: Pengambilan kas Pabrik Mayora - Pembelian Biskuit",
    )
    ...
    def submit_form(e):
        try:
            ...
            if not is_edit:
                ...
                success_msg = "Catatan pengambilan kas pabrik berhasil disimpan!"
            else:
                update_pengambilan_pabrik(
                ...
                success_msg = "Catatan pengambilan kas pabrik berhasil diperbarui!"
    ...
    form_dialog_title = ft.Text("Tambah Pengambilan Kas Pabrik", weight=ft.FontWeight.W_500)
    ...
    def open_edit_dialog(item):
        ...
        form_dialog_title.value = f"Edit Pengambilan Kas Pabrik #{item['id']}"
    ...
    def confirm_delete(e):
        try:
            delete_pengambilan_pabrik(del_target_id["id"])
            close_dialog()
            refresh_table_content()
    ...
    def open_delete_dialog(item):
        ...
        del_msg.value = f"Apakah Anda yakin ingin menghapus data pengambilan pabrik '{item['keterangan']}' senilai {rp(item['nominal'])}?"
    ...
    filter_card = ft.Container(
        content=ft.Column([
            ft.Row([
                ...
                ft.Text("Filter Data Pengambilan Pabrik", weight=ft.FontWeight.W_500, size=15),
    ...
    def build_table_rows(items):
        ...
        for it in items:
            ...
            cells = [ft.DataCell(ft.Text(tgl_str, size=13))]
            if is_pusat:
            ...
            cells.extend([
                ...
                ft.DataCell(
                    ft.Row([
                        ft.IconButton(ft.Icons.EDIT, icon_size=18, tooltip="Edit Catatan", on_click=lambda e, item=it: open_edit_dialog(item)),
                        ft.IconButton(ft.Icons.DELETE, icon_size=18, icon_color=ft.Colors.RED_400, tooltip="Hapus Catatan", on_click=lambda e, item=it: open_delete_dialog(item)),
                    ], spacing=2)
                ),
            ])
    ...
    def refresh_table_content():
        ...
        metric_total_card.content = metric_card(page, "Total Pengambilan Pabrik", rp(total_sum), light_color=ft.Colors.INDIGO_50, light_text_color=ft.Colors.INDIGO_900, dark_color=ft.Colors.INDIGO_900, dark_text_color=ft.Colors.INDIGO_100)
        metric_count_card.content = metric_card(page, "Jumlah Transaksi", f"{total_trx} Transaksi", light_color=ft.Colors.BLUE_50, light_text_color=ft.Colors.BLUE_900, dark_color=ft.Colors.BLUE_900, dark_text_color=ft.Colors.BLUE_100)
        metric_avg_card.content = metric_card(page, "Rata-rata Transaksi", rp(avg_sum), light_color=ft.Colors.ORANGE_50, light_text_color=ft.Colors.ORANGE_900, dark_color=ft.Colors.ORANGE_900, dark_text_color=ft.Colors.ORANGE_100)
        metric_max_card.content = metric_card(page, "Transaksi Terbesar", rp(max_sum), light_color=ft.Colors.TEAL_50, light_text_color=ft.Colors.TEAL_900, dark_color=ft.Colors.TEAL_900, dark_text_color=ft.Colors.TEAL_100)
        ...
        if not items:
            table_container.content = ft.Container(
                content=ft.Column([
                    ...
                    ft.Text("Tidak ada data pengambilan kas pabrik", size=15, weight=ft.FontWeight.W_500),
            ...
            columns = [ft.DataColumn(ft.Text("Tanggal"), on_sort=lambda e: on_sort_tanggal(0, e.ascending))]
            if is_pusat:
            ...
            dt = ft.DataTable(sort_column_index=0, sort_ascending=(filter_state["sort_order"] == "asc"), columns=columns, rows=build_table_rows(items), border=ft.Border.all(0.5, ft.Colors.GREY_700 if is_dark else ft.Colors.GREY_200), border_radius=10, heading_row_color=ft.Colors.GREY_800 if is_dark else ft.Colors.GREY_100, show_bottom_border=True)
    ...
    async def export_pdf(e):
        ...
        nama_file_default = f"Laporan_Pengambilan_Pabrik_{today.strftime('%Y%m%d')}.pdf"
        try:
            if page.platform == ft.PagePlatform.ANDROID or page.platform == ft.PagePlatform.IOS:
                pdf_bytes = generate_pengambilan_pabrik_pdf(items, filter_info, is_pusat=is_pusat, output_path=None)
                save_path = await export_picker.save_file(dialog_title="Simpan Laporan Pengambilan Pabrik PDF", file_name=nama_file_default, file_type=ft.FilePickerFileType.CUSTOM, allowed_extensions=["pdf"], src_bytes=pdf_bytes)
            else:
                save_path = await export_picker.save_file(dialog_title="Simpan Laporan Pengambilan Pabrik PDF", file_name=nama_file_default, file_type=ft.FilePickerFileType.CUSTOM, allowed_extensions=["pdf"])
                if save_path and not save_path.lower().endswith(".pdf"): save_path += ".pdf"
                if save_path: generate_pengambilan_pabrik_pdf(items, filter_info, is_pusat=is_pusat, output_path=save_path)
            
            if save_path:
                log_activity(actor.get("id"), actor.get("username", "user"), "CREATE", "export_pdf", filter_state.get("cabang_id") or 0, f"Export PDF Pengambilan Kas Pabrik ({periode_str})", filter_state.get("cabang_id"))
    ...
    body = ft.Column([
        ft.Row([
            ft.Column([
                ft.Text("Pengambilan Kas Pabrik", size=22, weight=ft.FontWeight.W_600),
                ft.Text("Pencatatan pengambilan dana kas untuk operasional atau pengadaan barang pabrik/supplier.", size=13, color=ft.Colors.GREY_500),
            ], expand=True),
            ft.OutlinedButton("Export ke PDF", icon=ft.Icons.PICTURE_AS_PDF, on_click=export_pdf),
            ft.ElevatedButton("Catat Pengambilan", icon=ft.Icons.ADD, on_click=open_add_dialog, bgcolor=ft.Colors.INDIGO_700, color=ft.Colors.WHITE),
        ...
        ft.Row([ft.Text("Daftar Catatan Pengambilan Kas Pabrik", size=16, weight=ft.FontWeight.W_500, expand=True)]),
        ft.Container(height=8),
        table_container,
