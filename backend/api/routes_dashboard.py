...
def cabang_summary(user: dict = Depends(get_current_user)):
...
def cabang_breakdown(user: dict = Depends(get_current_user)):
    ...
    for cabang_id, nama_cabang, modal, omzet, barang in rows:
        ...
        breakdown.update({"cabang_id": cabang_id, "nama_cabang": nama_cabang})
        result.append(breakdown)
    return result


@router.get("/monthly-trend")
def monthly_trend(cabang_id: int | None = None, limit_months: int = 6, user: dict = Depends(get_current_user)):
    if cabang_id is not None:
        assert_cabang_access(user, cabang_id)
    elif user["role"] != "admin":
        cabang_id = user["cabang_id"]
