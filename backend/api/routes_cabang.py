...
from auth.dependencies import require_pusat_admin
...
def check_cabang_name_exists(
    nama: str,
    exclude_id: int | None = None,
    user: dict = Depends(require_pusat_admin),
):
    return {
        "exists": cabang_repo.cabang_name_exist(nama, exclude_id)
    }
...
def list_cabang(
    ...
    rows = (
        cabang_repo.get_active_cabang()
        if active_only
        else cabang_repo.get_all_cabang()
    )
    return rows
...
def create_cabang(
    ...
    if cabang_repo.cabang_name_exist(body.nama_cabang):
        raise HTTPException(
            status_code=409,
            detail="Nama cabang sudah dipakai",
        )

    new_id = cabang_repo.create_cabang(
        body.nama_cabang,
        body.alamat,
    )
    log_activity(
...
def update_cabang(
    ...
    if cabang_repo.cabang_name_exist(
        body.nama_cabang,
        exclude_id=cabang_id,
    ):
        raise HTTPException(
            status_code=409,
            detail="Nama cabang sudah dipakai",
