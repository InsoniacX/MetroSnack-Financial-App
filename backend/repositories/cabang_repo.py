...
def create_cabang(nama_cabang, alamat):
    return execute(
        ...
        (nama_cabang, alamat), returning=True,
    )

def update_cabang(cabang_id, nama_cabang, alamat):
    execute("UPDATE cabang SET nama_cabang=%s, alamat=%s WHERE id=%s", (nama_cabang, alamat, cabang_id))

def set_cabang_aktif(cabang_id, aktif):
    execute("UPDATE cabang SET aktif=%s WHERE id=%s", (aktif, cabang_id))

def cabang_name_exist(nama_cabang, exclude_id=None):
    if exclude_id:
        row = fetch_one("SELECT id FROM cabang WHERE nama_cabang=%s AND id != %s", (nama_cabang, exclude_id))
    else:
        row = fetch_one("SELECT id FROM cabang WHERE nama_cabang=%s", (nama_cabang,))
